...
export async function fetchAllCoordinators(): Promise<Coordinator[]> {
    ...
    const getParam = (name: string): string | undefined => {
        const match = textResult.match(new RegExp(`var\\s+${name}\\s*=\\s*['"]([^'"]+)['"];`));
    ...
    if (!officeRes.ok) throw new Error(`Failed to fetch office IDs: ${officeRes.statusText}`);
    ...
    if (!coordinatorRes.ok) throw new Error(`Failed to fetch coordinators: ${coordinatorRes.statusText}`);
