...
const BASE_URL =
  ...
  "https://ws-4u4mlvrvn79j5kvg.ap-southeast-1.maas.aliyuncs.com/compatible-mode/v1";
...
function resolveImageUrl(source, isUrl) {
  ...
  return `data:image/${mimeMap[ext] || "jpeg"};base64,${data.toString(
    "base64"
  )}`;
}
      ...
      (res) => {
        ...
        res.on("end", () => {
          if (res.statusCode >= 400)
            return reject(
              new Error(`API ${res.statusCode}: ${data.slice(0, 300)}`)
            );
          try {
