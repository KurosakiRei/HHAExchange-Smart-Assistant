        ...
        return new Promise((resolve) => {
            const parseRows = () => {
                try {
                    ...
                    if (!table) {
                        console.warn("[PrebillingTableParser] parseAllRows: table not found");
                        resolve([]);
                        return;
                    }
                    const tbody = table.querySelector("tbody");
                    ...
                    console.log(`[PrebillingTableParser] parseAllRows: found ${records.length} rows`);
                    resolve(records);
                }
                catch (error) {
    ...
    static async executePOCClean(task) {
        ...
        for (const row of rows) {
            ...
            if (admissionId === task.admissionId &&
                ...
                console.log(`[CleaningController] [Match-1] Matched ${task.patientName} (${task.admissionId}) on ${task.visitDate} at ${task.scheduledTime}`);
                targetRow = row;
                foundByMatch = true;
                break;
            }
        ...
        if (!foundByMatch) {
            console.warn(`[CleaningController] 4-field match failed for ${task.patientName} (${task.admissionId}). Trying 3-field match (admissionId + patientName + visitDate)...`);
            for (const row of rows) {
                const cells = row.querySelectorAll("td");
                if (cells.length < 14)
                    continue;
                ...
                if (admissionId === task.admissionId &&
                    patientName?.includes(task.patientName || "") &&
                    visitDate === task.visitDate) {
                    console.warn(`[CleaningController] [Match-2] Fallback matched by 3 fields (no scheduledTime): ${task.patientName} (${task.admissionId}) on ${task.visitDate}`);
                    targetRow = row;
                    foundByMatch = true;
                    break;
                }
        ...
        if (!foundByMatch) {
            console.warn(`[CleaningController] 3-field match also failed. Trying 2-field match (admissionId + visitDate)...`);
            for (const row of rows) {
                const cells = row.querySelectorAll("td");
                if (cells.length < 14)
                    continue;
                ...
                if (admissionId === task.admissionId && visitDate === task.visitDate) {
                    console.warn(`[CleaningController] [Match-3] Fallback matched by admissionId+visitDate only: ${task.patientName} (${task.admissionId}) on ${task.visitDate}`);
                    targetRow = row;
                    foundByMatch = true;
                    break;
                }
        ...
        if (!foundByMatch && task.rowIndex !== undefined && rows[task.rowIndex]) {
            console.warn(`[CleaningController] All field-matches failed. Falling back to rowIndex ${task.rowIndex} with date verification for ${task.patientName}`);
            const candidateRow = rows[task.rowIndex];
            ...
            if (candidateCells.length >= 14) {
                ...
                const candidateDate = candidateCells[0]?.textContent?.trim().replace(/\s+/g, " ") || "";
                if (candidateAdmId.includes(task.admissionId) &&
                    candidateDate === task.visitDate) {
                    console.warn(`[CleaningController] [Match-4] rowIndex ${task.rowIndex} verified by admissionId+visitDate`);
                    targetRow = candidateRow;
                }
                else {
                    // rowIndex 对应行不匹配：在全表中按 admissionId + visitDate 搜索
                    console.warn(`[CleaningController] rowIndex ${task.rowIndex} mismatch. Searching by admissionId+visitDate: ${task.admissionId} / ${task.visitDate}`);
                    const deepFallback = Array.from(rows).find((r) => {
                        const rCells = r.querySelectorAll("td");
                        if (rCells.length < 14)
                            return false;
                        const idStr = rCells[1]?.textContent?.trim() || "";
                        const dateStr = rCells[0]?.textContent?.trim().replace(/\s+/g, " ") || "";
                        return (idStr.includes(task.admissionId) && dateStr === task.visitDate);
                    });
                    if (deepFallback) {
                        console.warn(`[CleaningController] [Match-4b] Found by admissionId+visitDate scan`);
                        targetRow = deepFallback;
                    }
                    else {
                        console.error(`[CleaningController] All fallbacks exhausted. Row not found for ${task.patientName} (${task.admissionId}) on ${task.visitDate}`);
                        targetRow = null;
                    }
                }
            }
...
class CleanerTab extends BaseTab {
    ...
    async analyzePrebillingTable() {
        ...
        if (this._prebillingAnalyzing) {
            console.log("[CleanerTab] analyzePrebillingTable already running, skipping concurrent call");
            return;
        }
        ...
        try {
            ...
            const selectedFeatures = new Set(Array.from(this.selectedIndices)
                .map((i) => {
...
const TimesheetNotificationTemplate_DEFAULT_CONFIG = {
    recipientName: "Mariana",
    to: "Mzlotar@AlwaysNY.net",
    cc: "",
};
class TimesheetNotificationTemplate {
    constructor() {
        this.pageChangeHandler = null;
    ...
    loadConfig() {
        try {
            const stored = GM_getValue(GM_KEY, "");
            if (stored) {
                return { ...TimesheetNotificationTemplate_DEFAULT_CONFIG, ...JSON.parse(stored) };
            }
        }
        catch (_) {
            // ignore parse errors
        }
        return { ...TimesheetNotificationTemplate_DEFAULT_CONFIG };
    }
    saveConfig(config) {
    ...
    `;
        ...
        const recipientInput = overlay.querySelector("#ts-recipient-name");
        const toInput = overlay.querySelector("#ts-to");
        const ccInput = overlay.querySelector("#ts-cc");
        const saveBtn = overlay.querySelector("#ts-save-config");
        let currentSaved = { ...savedConfig };
        const checkDirty = () => recipientInput.value !== currentSaved.recipientName ||
        ...
        let currentRecords = [];
        const searchInput = overlay.querySelector("#ts-search");
        searchInput.addEventListener("input", () => {
            currentSearchTerm = searchInput.value;
            this.renderVisitList(overlay.querySelector("#ts-visit-list"), currentRecords, currentSearchTerm, checkDirty, () => currentSaved, overlay);
        });
        // ── Initial load + polling ───────────────────────────────────────────
        const refreshList = async () => {
            const listEl = overlay.querySelector("#ts-visit-list");
            if (!listEl)
                return;
            if (PageDetector.getCurrentPageType() !== "PREBILLING") {
            ...
            try {
                ...
                this.renderVisitList(listEl, currentRecords, currentSearchTerm, checkDirty, () => currentSaved, overlay);
            }
            catch (_) {
    ...
    // ─── Visit List Renderer ─────────────────────────────────────────────────
    renderVisitList(listEl, records, searchTerm, isDirty, getSavedConfig, overlay) {
        const lower = searchTerm.toLowerCase();
        const filtered = searchTerm
            ? records.filter((r) => r.patientName.toLowerCase().includes(lower) ||
                r.admissionId.toLowerCase().includes(lower))
            : records;
        if (filtered.length === 0) {
            listEl.innerHTML = `<div class="timesheet-no-data">${records.length === 0 ? "暂无访问记录" : "无匹配结果"}</div>`;
            return;
        }
        listEl.innerHTML = "";
        ...
        <div class="timesheet-visit-info">
          <span class="timesheet-patient">${this.escapeHtml(record.patientName)} | ${this.escapeHtml(record.admissionId)}</span>
          <span class="timesheet-details">
            <span class="timesheet-date">📅 ${this.escapeHtml(record.visitDate)}</span>
            <span class="timesheet-time">🕐 ${this.escapeHtml(record.scheduledTime)}</span>
          </span>
        </div>
        <button class="template-modal-btn btn-save timesheet-send-btn">▶ Outlook</button>
      `;
            const sendBtn = card.querySelector(".timesheet-send-btn");
    ...
    // ─── Send ────────────────────────────────────────────────────────────────
    handleSend(record, isDirty, getSavedConfig, overlay) {
        if (isDirty()) {
    ...
    doSend(record, config, overlay) {
        const normalizeAddresses = (raw) => raw
            .split(/[,;]/)
            .map((s) => s.trim())
            .filter(Boolean)
            .join(",");
    ...
    startPolling(callback) {
        this.stopPolling();
        this.pollingTimer = setInterval(callback, TimesheetNotificationTemplate.POLLING_INTERVAL);
    }
    stopPolling() {
        if (this.pollingTimer) {
            clearInterval(this.pollingTimer);
...
TimesheetNotificationTemplate.POLLING_INTERVAL = 5000;

;// ./src/js/services/PatientCalendarApiProvider.ts
/**
...
class PatientCalendarApiProvider {
    /**
     * 获取 Calendar API 所需参数，优先从 Calendar iframe 读取，否则 GM_fetch 解析
     */
    static async getParams(patientId) {
    ...
    static tryFromIframe(patientId) {
        try {
            const iframe = document.getElementById("iframefrmRightSide");
            if (!iframe)
                return null;
            const src = iframe.src || "";
            if (!src.includes("InternalPatientCalendarDetails_ns.aspx"))
                return null;
            const iDoc = iframe.contentDocument || iframe.contentWindow?.document;
            if (!iDoc)
                return null;
            const get = (id) => iDoc.getElementById(id)?.value ?? "";
    ...
    static async fetchFromCalendarPage(patientId) {
        ...
        const officeId = urlParams.get("office") ||
            ...
            document.getElementById("hdnvendorID")
                ?.value ||
            "";
        const calendarUrl = `${baseUrl}/Patient/InternalPatientCalendarDetails_ns.aspx` +
            `?PatientID=${encodeURIComponent(patientId)}&dt=${Date.now()}` +
            (officeId ? `&office=${encodeURIComponent(officeId)}` : "");
        const r = (await GM_fetch(calendarUrl, { method: "GET" }));
        const html = await r.rawBody.text();
        const getInput = (id) => {
        ...
        if (!hhwsPath || !appSecret || !userID) {
            throw new Error("[PatientCalendarApiProvider] Failed to extract required Calendar API params from HTML");
        }
        return {
            hhwsPath,
            appName: appName || "HHAeXchange",
...
// ─── Types ───────────────────────────────────────────────────────────────────
const PatientVacationTemplate_GM_KEY = "hha_builtin_patient_vacation_config";
const LOADING_TIMEOUT_MS = 180000; // 3 minutes
const PatientVacationTemplate_DEFAULT_CONFIG = {
...
class PatientVacationTemplate {
    ...
    loadConfig() {
        try {
            const stored = GM_getValue(PatientVacationTemplate_GM_KEY, "");
            if (stored)
                return { ...PatientVacationTemplate_DEFAULT_CONFIG, ...JSON.parse(stored) };
        }
        catch (_) {
            // ignore
        }
        return { ...PatientVacationTemplate_DEFAULT_CONFIG };
    }
    saveConfig(config) {
        GM_setValue(PatientVacationTemplate_GM_KEY, JSON.stringify(config));
    }
    // ─── Entry Card ────────────────────────────────────────────────────────────
    renderEntryCard(container) {
        if (this.pageChangeHandler) {
    ...
    async startFlow() {
        ...
        if (result === "timeout") {
            ...
            data.toastWarnings.push("⚠️ 数据获取超时（3分钟），部分字段可能需要手动填写");
        }
        else {
            data = result;
        }
    ...
    async fetchAllData() {
        ...
        let apiParams = null;
        try {
            apiParams = await PatientCalendarApiProvider.getParams(patientId === "[无法获取]" ? "" : patientId);
        }
        catch (e) {
        ...
        if (apiParams) {
            try {
                ...
                if (vacList.length === 0) {
                    toastWarnings.push("⚠️ 未找到 vacation 记录，请先在 Vacation Tab 添加");
                    failedFields.push("Vacation 日期");
                }
                else {
                    const rec = vacList[0];
                    vacationStart = this.formatDate(rec.StartDate || rec.CalenderStartDate);
                    ...
                    if (endDateObj && endDateObj < new Date()) {
                        toastWarnings.push(`⚠️ 检测到的 vacation 记录已过期（${vacationEnd}），请检查日期是否正确`);
                    }
                }
            }
            catch (e) {
        ...
        if (apiParams && vacationStart !== "[无法获取]") {
            try {
                lastServiceDate = await this.calcLastServiceDate(apiParams, vacationStart);
            }
            catch (e) {
        ...
        if (lastServiceDate === "[无法获取]" && resumptionDate === "[无法获取]") {
            if (!failedFields.some((f) => f === "最后服务日") &&
    ...
    async getVacationInfo(params) {
        ...
        const baseUrl = ApiParamProvider.getTenantBaseUrl().replace(/https:\/\/app\.hhaexchange\.com/, "https://app.hhaexchange.com");
    ...
    // ─── Last Service Date ─────────────────────────────────────────────────────
    async calcLastServiceDate(params, vacationStart) {
    ...
    // ─── Resumption Date ───────────────────────────────────────────────────────
    async calcResumptionDate(params, vacationEnd) {
        ...
        if (after.length > 0)
            return after[0].VisitDate;
        // Strategy 2: Master Week fallback
        return this.calcResumptionFromMasterWeek(params.patientID, endDate);
    }
    async calcResumptionFromMasterWeek(patientId, vacationEndDate) {
        const baseUrl = ApiParamProvider.getTenantBaseUrl();
        const url = `${baseUrl}/Patient/InternalPatientMasterWeekIFrame_ns.aspx?PatientID=${encodeURIComponent(patientId)}`;
    ...
    parseMasterWeekServiceDays(html) {
        ...
        try {
            ...
            for (const table of Array.from(doc.querySelectorAll("table"))) {
                ...
                const colToDow = new Map();
                Array.from(rows[0].querySelectorAll("td,th")).forEach((cell, idx) => {
                    const txt = (cell.textContent ?? "").trim().toLowerCase();
                    if (txt in dayNameToDoW)
                        colToDow.set(idx, dayNameToDoW[txt]);
                });
                if (colToDow.size === 0)
                    continue;
                ...
                for (const row of rows) {
                    ...
                    if (!cells[0]?.textContent?.toLowerCase().includes("hours"))
                        continue;
    ...
    openModal(data) {
        ...
        const subjectPreFill = data.vacationStart !== "[无法获取]" && data.vacationEnd !== "[无法获取]"
          ...
          <div class="pv-config-section">
            <div class="pv-config-row">
              ...
              <input type="text" class="pv-config-input" id="pv-to"
                value="${this.escapeHtml(config.to)}" placeholder="收件人地址，逗号或分号分隔">
            </div>
            <div class="pv-config-row">
              <label class="pv-config-label">CC:</label>
              <input type="text" class="pv-config-input" id="pv-cc"
                value="${this.escapeHtml(config.cc)}" placeholder="抄送地址，逗号或分号分隔">
            </div>
          </div>
    ...
    buildBodyHtml(data) {
        return `Hello,<br><br>The patient will be on vacation from ${this.escapeHtml(data.vacationStart)} to ${this.escapeHtml(data.vacationEnd)}. The last day of service will be ${this.escapeHtml(data.lastServiceDate)} and the resumption of service will be ${this.escapeHtml(data.resumptionDate)}.`;
    }
    setupModalHandlers(overlay, initialConfig, data) {
        const toInput = overlay.querySelector("#pv-to");
        const ccInput = overlay.querySelector("#pv-cc");
        const saveBtn = overlay.querySelector("#pv-save-config");
        const subjectInput = overlay.querySelector("#pv-subject");
        const bodyEditor = overlay.querySelector("#pv-body-editor");
        const toolbar = overlay.querySelector("#pv-toolbar");
        let currentSaved = { ...initialConfig };
        // ── Config dirty tracking ────────────────────────────────────────────
        const checkDirty = () => toInput.value !== currentSaved.to || ccInput.value !== currentSaved.cc;
        const updateSaveBtn = () => {
            saveBtn.disabled = !checkDirty();
        };
        toInput.addEventListener("input", updateSaveBtn);
        ...
        // ── Copy Split Button ─────────────────────────────────────────────────
        const dropdown = overlay.querySelector("#pv-split-dropdown");
    ...
    doSendOutlook(subjectInput, bodyEditor, config) {
    ...
    showConsolidatedToasts(data) {
        ...
        if (fieldWarnings.length >= 2) {
            setTimeout(() => this.showToast(`⚠️ 以下字段无法自动获取，请手动填写：${fieldWarnings.join("、")}`, "warning"), 200);
        }
        else if (fieldWarnings.length === 1) {
            if (fieldWarnings[0] === "最后服务日" &&
                !data.toastWarnings.some((w) => w.includes("最后服务日"))) {
                setTimeout(() => this.showToast("⚠️ 无法推算最后服务日，请手动填写", "warning"), 200);
            }
            else if (fieldWarnings[0] === "Resumption 日期" &&
                !data.toastWarnings.some((w) => w.includes("Resumption"))) {
                setTimeout(() => this.showToast("⚠️ 无法推算 Resumption 日期，请手动填写", "warning"), 200);
            }
        }
    }
    showToast(message, type) {
...
;// ./src/js/utils/IDBHandleStore.ts
/**
...
// ─── Types ────────────────────────────────────────────────────────────────────
const EodReportTemplate_GM_KEY = "hha_builtin_eod_report_config";
...
const EodReportTemplate_DEFAULT_CONFIG = {
    recipientName: "Reggie",
    recipientEmail: "rthomas@AlwaysNY.net",
};
// ─── Main Class ───────────────────────────────────────────────────────────────
class EodReportTemplate {
    // ─── Config ─────────────────────────────────────────────────────────────
    loadConfig() {
        try {
            const stored = GM_getValue(EodReportTemplate_GM_KEY, "");
            if (stored)
                return { ...EodReportTemplate_DEFAULT_CONFIG, ...JSON.parse(stored) };
        }
        catch (_) {
            // ignore
        }
        return { ...EodReportTemplate_DEFAULT_CONFIG };
    }
    saveConfig(config) {
        GM_setValue(EodReportTemplate_GM_KEY, JSON.stringify(config));
    }
    // ─── Entry Card ──────────────────────────────────────────────────────────
    renderEntryCard(container) {
        const card = document.createElement("div");
    ...
    async startFlow() {
        ...
        if (!handle) {
            ...
            if (permission !== "granted") {
                ...
                this.showToast("⚠️ 请在浏览器权限横幅中点击「允许」以访问文件夹", "warning");
                return;
            }
        }
    ...
    openModal(handle, initialFiles) {
        ...
        const bodyPreFill = `Hello ${this.escapeHtml(config.recipientName)},<br><br>Please see attached.`;
        const overlay = document.createElement("div");
          ...
          <div class="eod-folder-section">
            <div class="eod-folder-header">
              <span class="eod-folder-label">
                📁 <span class="eod-folder-hint">已选择文件夹：</span><span id="eod-folder-name">${this.escapeHtml(handle.name)}</span>
              </span>
    ...
    renderFileListHtml(files) {
        ...
        return files
            .map((f, i) => `<div class="eod-file-item" data-index="${i}">
            <span class="eod-file-icon">📄</span>
            <span class="eod-file-name">${this.escapeHtml(f.name)}</span>
            <span class="eod-file-date">修改于 ${this.formatDate(f.lastModified)}</span>
    ...
    // ─── Modal Handlers ───────────────────────────────────────────────────────
    setupModalHandlers(overlay, currentFiles, currentHandle, initialConfig) {
        const nameInput = overlay.querySelector("#eod-recipient-name");
        const emailInput = overlay.querySelector("#eod-recipient-email");
        const saveConfigBtn = overlay.querySelector("#eod-save-config");
        const outlookBtn = overlay.querySelector("#eod-outlook");
        const fileListEl = overlay.querySelector("#eod-file-list");
        const folderNameEl = overlay.querySelector("#eod-folder-name");
        let savedConfig = { ...initialConfig };
        let handle = currentHandle;
        // ── Config dirty tracking ─────────────────────────────────────────────
        const checkDirty = () => nameInput.value !== savedConfig.recipientName ||
        ...
        // ── Toolbar ───────────────────────────────────────────────────────────
        const toolbar = overlay.querySelector("#eod-toolbar");
        toolbar.addEventListener("mousedown", (e) => {
            const btn = e.target.closest("[data-cmd]");
            if (btn) {
                e.preventDefault();
        ...
        outlookBtn.addEventListener("click", async () => {
            const bodyEditor = overlay.querySelector("#eod-body-editor");
            const subjectInput = overlay.querySelector("#eod-subject");
...
class DocumentDropzone {
                ...
                if (result.saveToLocal) {
                    try {
                        ...
                        if (showSaveFilePicker) {
                            ...
                            console.log("[Epic 14] Saved file via Save As dialog:", newFile.name);
                        }
                        else {
                            ...
                            a.click();
                            URL.revokeObjectURL(url);
                            console.log("[Epic 14] Triggered local download for:", newFile.name);
                        }
                    }
                    catch (e) {
     ...
     */
    findAddDocumentModal() {
        // Find any heading element whose trimmed text is "Add Document"
        const headings = document.querySelectorAll("h1, h2, h3, h4, h5, legend, .modal-title, strong");
        ...
        return (header.closest(".reveal, [role='dialog'], .modal") ??
            header.closest("table, div"));
    }
    /**
    ...
    unbindDropzone() {
        ...
        if (this.overlay && this.overlay.parentNode) {
            const container = this.overlay.parentNode;
    ...
    promptForRename(originalName) {
      ...
      `;
            ...
            const saveLocalCheck = modal.querySelector("#save-local-check");
    ...
    attachFileToInput(file) {
        const fileInput = document.querySelector('input[type="file"][name="fuUpload2"]') ||
    ...
    autoFillMetadata(fileName) {
        ...
        const descTextarea = document.getElementById("attachedDocumentDescription");
        if (descTextarea) {
            const lastDotIndex = fileName.lastIndexOf(".");
...
;// ./package.json
const package_namespaceObject = {"rE":"3.16.1"};
;// ./src/index.ts
...
async function checkAndResumeCleaningTasks() {
    ...
    if (pageType === "DETAIL") {
        ...
        if (queue &&
            ...
            if (currentTask.admissionId &&
                !pageText.includes(currentTask.admissionId)) {
                console.warn(`[Epic 11] Patient mismatch! Expected ${currentTask.admissionId} but not found in page. This usually means the previous save failed with a validation error. Reloading top window to recover...`);
...
function handleConfirmationDialog(retryCount = 0) {
    ...
    if (okButton) {
        ...
        console.log("[Epic 11] No confirmation dialog found after retries. Assuming silent success. Forcing top window reload...");
        try {
            if (window.top)
                window.top.location.reload();
            else
