...
### 强制技术标准
- [ ] 构建产物 `dist/index.prod.user.js` **必须**是单一文件，绝对不允许出现 Webpack 打包产生的任何外部 `.chunk.js` 散落。
- [ ] 在 `metadata.cjs` 中必须申请 `@grant GM.addElement`。

### 功能体验标准
- [ ] 开启插件并在 HHA 选择某个病人模板，点击“发送到 Outlook”后。
- [ ] 成功自动呼出 Outlook 写信面板。
- [ ] Oulook 上的 To、CC、Subject、Body 等字段成功被正确回填，并且没有破坏 Outlook 原有的保存收件人、换行、或样式格式。
- [ ] 浏览器的 Console 不再飘红显示 CSP 拦截错误 (`Refused to execute inline script because it violates CSP`)。

### 降级处理标准
- [ ] 若使用者所安装的 Tampermonkey 版本过于古老，不支持 `GM.addElement` API，能够体面地检测到并在 UI 上提示用户升级（不让整个脚本崩溃）。
...
**任务列表**:
- [ ] 更新 `config/metadata.cjs` 的 `@grant` 列表，加入必需的 `GM.addElement`（或其兼容形式）。
- [ ] 新建 `src/js/services/OutlookDOMControllerPayload.ts`，并写入一个 `export const` 字面量字符串占位（内含只用来 console.log 的探路代码）。
- [ ] 新建 `src/js/services/CSPBypassInjector.ts` 提供工具方法，负责检查 API 的存在性并执行 `GM.addElement` 注入。
...
**任务列表**:
- [ ] 在 `OutlookDOMControllerPayload.ts` 的长字符串里完成基于 `document.querySelector` 的查元素操作。
- [ ] 实现原生的 React/Angular 事件触发机制（因为 Outlook 是单页应用，简单的 `.value = 'x'` 可能无法更新其内部状态，需要触发原生的 `input` 或 `change` 事件）。
- [ ] 建立 Payload 和适配器外壳之间基于 `window.CustomEvent` 的握手（用于发任务和回传状态）。
...
**任务列表**:
- [ ] 清理老版 `OutlookAdapter.ts` 中废弃阻塞的纯 DOM 操作，转为调用 Injector 注入。
- [ ] 处理监听逻辑，监听 `hha_mail_service_bus` 的任务变化。
- [ ] 接住 Payload 抛出的 `CustomEvent` 状态结果（成功/失败），再写回存储让 HHA 端知道。
- [ ] 编译测试，并最终验证生成文件是不是 100% 单纯唯一的文件。

---

*（Epic 定义完毕，在审批后将依序进入实质开发阶段）*
