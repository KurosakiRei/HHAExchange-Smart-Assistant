...
| Story | 标题 | 优先级 | 工时 | 状态 |
|-------|------|--------|------|------|
| 9.1 | 表格排序功能 | High | 4h | 🔲 待开始 |
| 9.2 | 表格可选中与复制 | Medium | 1h | 🔲 待开始 |
| 9.3 | 电话号码列功能实现 | High | 6h | 🔲 待开始 |
| 9.4 | 导出功能修复 | High | 2h | 🔲 待开始 |
| 9.5 | 查看病人详情功能 | Medium | 3h | 🔲 待开始 |
| 9.6 | 快速创建 QA Note 弹窗 | Medium | 3h | 🔲 待开始 |
...
### 验收标准
- [ ] 将 "Admission ID" 列名改为 "ID"
- [ ] 以下列支持点击排序：
  - [ ] ID 列 - 按字母 A-Z 排序
  - [ ] 病人姓名列 - 按字母 A-Z 排序
  - [ ] 上次 QA 列 - 按天数大小排序
- [ ] 排序状态三态循环：原序 → 升序 → 降序 → 原序
- [ ] 可排序列的表头显示排序图标：
  - 原序状态：显示 ⇅ 或不显示图标
  - 升序状态：显示 ▲
  - 降序状态：显示 ▼
- [ ] 点击表头时排序状态切换，同时更新图标
- [ ] 排序时保持数据完整性（所有列数据同步排序）
- [ ] "解析到的QA日期" 和 "操作" 列不支持排序
...
### 验收标准
- [ ] 恢复第三列为"电话"列（替换当前的 debug 列）
- [ ] 通过 Admission ID 获取病人电话号码
- [ ] 单个电话号码：直接显示号码
- [ ] 多个电话号码：显示 "⋯" 图标，点击展开下拉框
- [ ] 下拉框显示所有电话号码
- [ ] 点击电话号码使用 `tel:` 协议发起拨打
- [ ] 所有电话号码可以选中复制
- [ ] 加载数据时显示 loading 状态
...
- [ ] 点击"导出"按钮显示格式选择下拉菜单
- [ ] 菜单显示在按钮下方正确位置
- [ ] 菜单包含 "📄 导出 CSV" 和 "📋 导出 JSON" 两个选项
- [ ] 点击选项触发对应的导出功能
- [ ] 点击菜单外区域关闭菜单
- [ ] 导出的文件名格式：`QA_Report_{Coordinator}_{Date}.{ext}`
- [ ] CSV 导出包含 UTF-8 BOM 以支持 Excel 中文显示
...
- [ ] "⋮" 操作菜单中的"📋 查看病人详情"可正常使用
- [ ] 点击后在新标签页打开正确的病人详情页
- [ ] 使用 Profile ID 构建 URL（非 Admission ID）
- [ ] 如果无法获取 Profile ID，显示错误提示
...
### 验收标准
- [ ] "⋮" 操作菜单中的"📝 快速创建 QA Note"打开弹窗
- [ ] 弹窗显示病人姓名和 Admission ID
- [ ] 弹窗显示预设的 QA 语句提示
- [ ] 弹窗包含可选的附加备注输入框
- [ ] "取消"按钮关闭弹窗
- [ ] "提交并关闭"按钮显示"功能开发中..."提示
