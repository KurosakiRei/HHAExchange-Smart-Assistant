...
- `skipped` 数量
- `failed` 数量
- 若失败存在，展示全部失败日期，并允许通过滚动列表查看完整明细
...
### D10：执行批量写入前必须二次确认，并显示待写入日期清单

**选定**：当用户在批量弹窗点击 `开始批量添加` 后，先弹出确认对话框，明确展示本次将要写入的日期列表；仅在用户再次确认后才发起批处理。

**理由**：

1. 批量写入属于不可逆业务操作，二次确认可降低误触风险；
2. 日期清单可让用户在最后一步校验范围与周几筛选结果；
3. 该确认不改变批处理主逻辑，只增加可控安全闸门。

### D11：在批量 Notes 弹窗内集成 Outlook 入口，复用 Timesheet 收件人配置

**选定**：在 `快速添加 Patient Visit Notes` 弹窗左下角提供 `Outlook` 按钮，点击后打开 Timesheet 邮件预览弹窗：

1. `To/CC` 默认读取 `TimesheetNotificationTemplate` 已保存配置（`hha_builtin_timesheet_config`）；
2. Subject 默认包含当前已选日期集合；同年多日期场景采用紧凑格式，仅在末尾保留年份；
3. 用户可编辑 Subject/Body 后发送到 Outlook。

**理由**：

1. 用户在同一工作流中往往会紧接着发送 Timesheet 邮件，入口前置可减少上下文切换；
2. 复用已有收件人配置可避免重复维护联系人；
3. 将选中日期直接注入 Subject，保证邮件与批量写入日期一致。
...
- 批量执行前可见性更高，误操作概率降低；
- 批量弹窗可直接进入 Outlook 邮件预览，减少跨功能跳转。
